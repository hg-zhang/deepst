...
from keras.layers import (Input, Activation, add, Dense, Reshape)
...
def ResUnits(residual_unit, nb_filter, repetations=1):
    def f(input):
        for i in range(repetations):
            init_subsample = (1, 1)
            input = residual_unit(
                nb_filter=nb_filter, init_subsample=init_subsample)(input)
        return input
...
def stresnet(
    ...
    for conf in [c_conf, p_conf, t_conf]:
        if conf is not None:
            ...
            # Conv1
            conv1 = Conv2D(
                kernel_size=(3, 3), filters=64, padding="same")(input)
            # [nb_residual_unit] Residual Units
            residual_output = ResUnits(
                _residual_unit, nb_filter=64,
                repetations=nb_residual_unit)(conv1)
            # Conv2
            activation = Activation('relu')(residual_output)
            conv2 = Conv2D(
                filters=nb_flow, kernel_size=(3, 3),
                padding="same")(activation)
