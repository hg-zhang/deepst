...
def timestamp2vec(timestamps):
    ...
    # vec = [time.strptime(t[:8], '%Y%m%d').tm_wday
    #    for t in timestamps]  # python2
    ret = []
    for i in vec:
...
def remove_incomplete_days(data, timestamps, T=48):
    ...
    while i < len(timestamps):
        if int(timestamps[i][8:]) != 1:
            i += 1
        elif i + T - 1 < len(timestamps) and int(
                timestamps[i + T - 1][8:]) == T:
...
def split_by_time(data, timestamps, split_timestamp):
    ...
    return (np.asarray(data_1), timestamps_1), (np.asarray(data_2),
                                                timestamps_2)
