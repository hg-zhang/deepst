...
def main():
    ...
    ts = time.time()
    fname = os.path.join(
        DATAPATH, 'CACHE', 'TaxiBJ_C{}_P{}_T{}.h5'.format(
    ...
    history = model.fit(
        ...
        verbose=1)
    model.save_weights(
        os.path.join('MODEL', '{}.h5'.format(hyperparams_name)),
        overwrite=True)
    ...
    score = model.evaluate(
        X_train, Y_train, batch_size=Y_train.shape[0] // 48, verbose=0)
    ...
    pickle.dump((history.history),
                open(
                    os.path.join(
                        path_result,
                        '{}.cont.history.pkl'.format(hyperparams_name)), 'wb'))
    model.save_weights(
        os.path.join('MODEL', '{}_cont.h5'.format(hyperparams_name)),
        overwrite=True)
    print(
